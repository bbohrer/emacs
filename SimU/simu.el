...
(defun advance-year ()
  (setq endowment (- endowment (academic-expenses)))
...
(defun fail-screen ()
    (force-erase-buffer)
    (widget-insert (format "You died of dysentery and/or bankruptcy.\nYou lasted until year %d.\n" year)))
...
(defun finance-screen ()
  ...
  (widget-insert (format "Total expenses: $%dK\n" (finance-expenses)))
  (widget-insert (format "Total profit: $%dK\n\n" (finance-profit)))
...
(defun finish-academics ()
  (setq teaching-profs (string-to-number (widget-value academics-teaching-profs)))
  (setq research-profs (string-to-number (widget-value academics-research-profs)))
  (setq prof-salary (string-to-number (widget-value academics-prof-salary)))
  (setq ta-budget (string-to-number (widget-value academics-ta-budget)))
  (setq ra-budget (string-to-number (widget-value academics-ra-budget)))
  (widget-delete academics-teaching-profs)
  (widget-delete academics-research-profs)
  (widget-delete academics-prof-salary)
  (widget-delete academics-ta-budget)
  (widget-delete academics-ra-budget))

(defvar teaching-profs)
(defvar research-profs)
(defvar prof-salary)
(defvar ta-budget)
(defvar ra-budget)
(setq teaching-profs 10)
(setq research-profs 10)
(setq prof-salary 80)
(setq ta-budget 40)
(setq ra-budget 10)

(defun academic-expenses ()
  (+ ta-budget ra-budget (* prof-salary (+ teaching-profs research-profs))))

(defun academics-screen ()
  (force-erase-buffer)
  (print-stats)
  (setq academics-teaching-profs (widget-create 'editable-field
                 :size 10
                 :format "Teaching Professors: %v \n\n"
                 (number-to-string teaching-profs)))
  (setq academics-research-profs (widget-create 'editable-field
                 :size 10
                 :format "Research Professors: %v \n\n"
                 (number-to-string research-profs)))
  (setq academics-prof-salary (widget-create 'editable-field
                 :size 10
                 :format "Avg. Professor Salary: %v \n\n"
                 (number-to-string prof-salary)))
  (setq academics-ta-budget (widget-create 'editable-field
                 :size 10
                 :format "TA Budget: %v \n\n"
                 (number-to-string ta-budget)))
  (setq academics-ra-budget (widget-create 'editable-field
                 :size 10
                 :format "RA Budget: %v \n\n"
                 (number-to-string ra-budget)))
  (widget-insert (format "Students per professor: %d\n\n" (/ enrollment teaching-profs)))
  (widget-insert (format "Academic expenses: %dK\n\n" (enrollment-expenses)))
  (widget-create 'push-button
                 :notify (lambda (&rest ignore)
                           (finish-academics)
                           (academics-screen))
